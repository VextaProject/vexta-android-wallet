...
object HeaderSync {
    ...
    fun loadCachedChain(context: Context): MutableList<ChainHeader> {
    ...
    private fun calculateAsertBits(chain: List<ChainHeader>): Long {
        ...
        if (shifts <= 0) {
            ...
            if (shifts >= 256) {
                ...
                val maxBeforeShift = POW_LIMIT.shiftRight(shifts.toInt())
                target =
    ...
    fun targetToCompact(targetValue: BigInteger): Long {
    ...
    fun readMessage(input: DataInputStream): Pair<String, ByteArray> {
    ...
    fun createVersionPayload(peerHost: String): ByteArray {
        val out = ByteArrayOutputStream()
    ...
    fun parseVersionPayload(payload: ByteArray): VersionInfo {
        val buffer = ByteBuffer.wrap(payload).order(ByteOrder.LITTLE_ENDIAN)
    ...
    fun writeVarString(out: ByteArrayOutputStream, value: String) {
        val bytes = value.toByteArray(Charsets.UTF_8)
        writeCompactSize(out, bytes.size)
        out.write(bytes)
    }

    fun readVarString(buffer: ByteBuffer): String {
    ...
    fun writeCompactSize(out: ByteArrayOutputStream, value: Int) {
        when {
    ...
    fun readCompactSize(buffer: ByteBuffer): Int {
    ...
    fun hashHexToWireBytes(hash: String): ByteArray =
    ...
    fun writeInt32LE(out: ByteArrayOutputStream, value: Int) {
        out.write(
    ...
    fun writeInt64LE(out: ByteArrayOutputStream, value: Long) {
        out.write(
